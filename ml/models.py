...
class RegressorModelParameters(object):
    ...
    """
    def __init__(self, w_init: np.ndarray = None, b_init: np.ndarray = None):
        self.w_init = w_init
        self.b_init = b_init
        self.w_min = None
        self.b_min = None
    ...
    def save_weights(self, filepath: Path, weights: np.ndarray, bias: np.ndarray, cost_history: np.ndarray) -> None:
        """
        ...
        with h5py.File(str(filepath), 'w') as file:
    ...
    def load_initial_weights(self, filepath: Path = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if filepath:
            if filepath.exists():
                with h5py.File(str(filepath), 'r') as model_file:
    ...
    def load_optimum_weights(self, filepath: Path = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        """
        # TODO: Create separate function returning cost history
        if filepath:
            if filepath.exists():
                with h5py.File(str(filepath), 'r') as model_file:
                    weights = model_file['min_weights'][()]
                    bias = model_file['min_bias'][()]
                    cost_history = model_file['cost_history'][()]
                    return weights, bias, cost_history
            else:
                raise FileNotFoundError
        else:
            return self.w_min, self.b_min, self.cost_history
